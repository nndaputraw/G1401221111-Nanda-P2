...
```{r}
ar1 <- arima.sim(list(order=c(1,0,0), ar=0.7), n=200) #Parameter (1,0,0), 1 untuk model AR, 0 untuk diferensi, 0, model MA
ar1
```
...
```{r}
acf(ar) 
# ACF tails of untuk AR, sedangkan cuts of untuk MA
```
...
```{r}
pacf(ar)
# PACF tails of untuk MA, sedangkan cuts of untuk AR
```
